module SN where
  ...
  sn-da : DecAtom
  sn-da = record { Y = ℕ ; Atom = Atom ; ⟦_⟧ₐ = ⟦_⟧ₐ ; ⟦_⟧ₐ? = ⟦_⟧ₐ? }
  ...
  -- In the substitution case
  elim-with-sub : {n : ℕ} → Sub n → List (Factor (suc n)) → List (Factor n)
  elim-with-sub sub fs = (ineqs sub) ++ (map (reduce-factor-sub sub) fs)

  -- Two cases:
  -- 1) There's a substitutable equality in the product => substitute.
  -- 2) There isn't => var₀ only non-trivial in inequalities => drop them.
  ...
  -- These are probably in the standard library.
  ...
  -- Case 1: there's a substitutable equality in the product
  ...
  -- Case 2: there are no substitutable equalities
  ...
  elim-prod-fwd : {n : ℕ} (fs : List (Factor (suc n)))
    → (e : Vec ℕ n)
    → ⟦ E (P.i fs) ⟧ e
    → ⟦ P.i (elim-prod fs) ⟧ e
  ...
  elim-prod-bwd : {n : ℕ} (fs : List (Factor (suc n)))
    → (e : Vec ℕ n)
    → ⟦ P.i (elim-prod fs) ⟧ e
    → ⟦ E (P.i fs) ⟧ e
  ...
  -- Lift to general single-step QE
  sn-qe : QE
  sn-qe = lift-prod-qe sn-prod-qe

  -- (Lift to full QE) + expose decidability
